...
public class UcanKus extends ApplicationAdapter {
	...
	public void create() {
		...
		birds = new Texture("aai.png");
		ari1 = new Texture("corel.png");
		ari2 = new Texture("corel.png");
		ari3 = new Texture("corel.png");
	...
	public void render() {
		...
		if (state == 1) {
			...
			for (int i=0;i< arisayisi;i++) {
				...
				batch.draw(ari2,arilarx[i],arilary[2][i],bw,bh);
		...
		if (by < 0) {
			...
			font1.draw(batch,"Oyun bitti",0,Gdx.graphics.getHeight()/2);
			if (flag1) {
				sound.play();
				flag1=false;
			}
			...
			font1.draw(batch,"Baslamak icin dokun",Gdx.graphics.getWidth()/3,Gdx.graphics.getHeight()/2);
			sound.play();
